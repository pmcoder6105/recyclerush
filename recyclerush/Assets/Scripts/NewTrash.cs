...
public class NewTrash : MonoBehaviour
{
    ...
    public void MakeNewTrash() {
        ...
        if (trashType == 1) {
            ...
            trash = Instantiate(recycle[variant], spawnPoint.position, Quaternion.identity);
        }
        if (trashType == 2) {
            //compost
            int variant = Random.Range(0,compost.Length);
            trash = Instantiate(compost[variant], spawnPoint.position, Quaternion.identity);
        }
        if (trashType == 3) {
            //landfill
            int variant = Random.Range(0,landfill.Length);
            trash = Instantiate(landfill[variant], spawnPoint.position, Quaternion.identity);
        }
    }
}